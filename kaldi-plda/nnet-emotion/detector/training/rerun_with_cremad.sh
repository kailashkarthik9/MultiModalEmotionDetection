...
. nnet-emotion/detector/training/cremad_settings.sh
...
if [ $stage -eq 0 ]; then
  ...
  nnet-emotion/detector/training/prepare_cremad.sh --stage 0
...
if [ $stage -eq 1 ]; then
  ...
  nnet-emotion/detector/training/prepare_cremad.sh --stage 1 --num_layers $num_layers --first_six_lr $first_six_lr
...
if [ $stage -eq 6 ]; then 
  ...
  rm -rf ${data_dir}/train_combined
  cp -r ${data_dir}/with_meld ${data_dir}/train_combined
  rm -rf ${data_dir}/with_meld
...
if [ $stage -eq 7 ]; then 
  ...
  rm -rf ${data_dir}/train_combined
  cp -r ${data_dir}/with_emovoxceleb ${data_dir}/train_combined
  rm -rf ${data_dir}/with_emovoxceleb
  echo "end stage $stage"
fi
